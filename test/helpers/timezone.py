...
class DateTime():
    ...
    @log_step()
    def select_ntp_server(self, server):
        self.browser.set_checked(f"#{self._step}-auto-date-time", True)
        self.browser.click("#anaconda-screen-date-time-ntp-toggle")
        self.browser.wait_visible("#anaconda-screen-date-time-ntp-server")
        self.browser.click(f"#anaconda-screen-date-time-ntp-server .pf-v6-c-menu__item-text:contains('{server}')")
        self.browser.wait_not_present("#anaconda-screen-date-time-ntp-server")

    @log_step()
    def check_ntp_server(self, server):
        self.browser.wait_in_text(
            f"#{self._step}-ntp-toggle .pf-v6-c-menu-toggle__text", server
        )
...
class TimeFormat():
    ...
    def set_time_format(self, fmt):
        # fmt: "24" or "ampm"
        self.browser.click(f"#{self._step}-time-format-{fmt}")

    @log_step()
    def check_time_format(self, fmt):
        checked = self.browser.get_checked(f"#{self._step}-time-format-{fmt}")
        assert checked, f"Expected radio #{self._step}-time-format-{fmt} to be checked"



# --- DBus Mixin (pro kontrolu na backendu) ---
class DateTimeDBus():
    ...
    def dbus_get_ntp_servers(self):
        out = self.machine.execute(f'busctl --address="{self._bus_address}" \
            get-property {self.TIMEZONE_SERVICE} {self.TIMEZONE_OBJECT_PATH} \
            {self.TIMEZONE_INTERFACE} TimeSources')
        return [x for x in out.split('"') if "." in x]
