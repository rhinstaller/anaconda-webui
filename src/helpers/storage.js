...
export const getOSDisks = (deviceData, osData) => {
    return osData.devices.v.map(deviceId => getDeviceAncestors(deviceData, deviceId))
            .reduce((disks, ancestors) => disks.concat(ancestors))
            .filter(dev => deviceData[dev].type.v === "disk")
...
export const isCompleteOSOnDisks = (deviceData, selectedDisks, osData) => {
    return getOSDisks(deviceData, osData).filter(disk => !selectedDisks.includes(disk)).length === 0;
};
