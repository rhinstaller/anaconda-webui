...
import { unitMultiplier } from "../../helpers/storage.js";
...
const getReclaimableSpaceFromAction = ({ action, devices, unappliedActions }) => {
    if (action === "remove") {
        return Object.keys(unappliedActions)
                .filter(device => unappliedActions[device].map(action => action.type).includes("remove"))
                .reduce((acc, device) => acc + devices[device].total.v - devices[device].free.v, 0);
    }

    if (action === "shrink") {
        return Object.keys(unappliedActions)
                .filter(device => unappliedActions[device].map(action => action.type).includes("shrink"))
