...
export const ReclaimSpaceModal = ({ onClose, onNext }) => {
    ...
    const onReclaim = async () => {
        // removeDevice API is only available in the automatic resizable storage partitioning module
        // which is available on the device tree of the created partitioning
...
const ReclaimFooter = ({ onClose, onReclaim, unappliedActions }) => {
    ...
    return (
        <Stack hasGutter>
            ...
            <ActionList>
                <Button isDisabled={status === "warning"} key="reclaim" variant="warning" onClick={onReclaim}>
                    {_("Reclaim space")}
                </Button>
