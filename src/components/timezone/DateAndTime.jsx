...
const DateAndTimeSection = ({ setSectionValid, timeFormat, timezone }) => {
    ...
    useEffect(() => {
        const updateTimeDateData = async () => {
            try {
                ...
                if (dtIso) setDatetime(DateTime.fromISO(dtIso, { zone: "utc" }).setZone(timezone));
    ...
    useEffect(() => {
        setSectionValid(autoDateTime || datetime.isValid);
    }, [autoDateTime, datetime, setSectionValid]);
